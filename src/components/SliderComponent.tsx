    ...
    <div className="xl:!max-w-[1350px] mx-auto md:mx-auto !max-w-[calc(100vw-10px)] p-3 pt-[80px] mt-[-40px]">
      <Slider {...sliderSettings} className="flex md:items-center">
        {sliderCards?.map((card, index) => (
          <div className="!flex max-md:flex-col !gap-6 p-4 md:p-[65px] bg-surface rounded-xl mb-8 border-[1px] border-[#393939]" key={index}>
              ...
              <div className="flex flex-col gap-6">
                {card?.organizationDetails?.map((item, index) => (
                  <div className="flex flex-col gap-3" key={index}>
                    <p className=" border-l-[3px] border-accent pl-2 text-[#606060] text-[18px] md:text-[22px] md:leading-[27px] leading-[23px]">
                      {item?.name}
                    </p>
                    <p className="text-content md:text-[22px] md:leading-[27px] text-[16px] leading-[20px] md:max-w-[80%]">
                      {item?.description}
                    </p>
                  </div>
                ))}
            ...
            <div className="flex flex-col gap-2" key={index}>
              <p className="text-[#606060] md:text-[22px] md:leading-[27px] text-[18px] leading-[23px] border-l-[3px] border-accent pl-2">
                Impact
              </p>
              <div className="flex max-md:flex-col md:flex-wrap md:items-center md:gap-4 gap-2 md:mt-[-38px]">
                {card?.impactDetails?.map((item, index) => (
                  ...
                  >
                    ...
                    <p className="text-content text-[16px] leading-[20px] md:text-[22px] md:leading-[27px] md:min-w-[223px]">
                      {item?.description}
                    </p>
                  </div>
                ))}
